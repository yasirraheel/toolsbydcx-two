// Cosmetic privacy for the connected Flow tab only. No account or project data
// is deleted or secured by these rules. Never run on Google's sign-in pages.
(() => {
  "use strict";
  const start = async () => {
    try {
  ...
  // Content scripts can be injected again when the extension is reloaded, or
  // when an already-open tab is brought back under management. Keep one
  // controller per page so an old observer/listener cannot retain old state.
  const instanceKey = "__bfFlowPrivacyController";
  const previous = globalThis[instanceKey];
  if (previous && typeof previous.destroy === "function") {
    try { previous.destroy(); } catch {}
  }

  const marker = "data-bf-flow-private";
  const rootMarker = "data-bf-flow-privacy";
  ...
  function isInteractive(element) {
    return element instanceof Element && element.matches(interactiveSelector);
  }

  function isHeaderElement(element) {
    return !!element.closest(headerSelector);
  }
  ...
  function unlock(control, original) {
    ...
    control.removeAttribute("data-bf-flow-account-locked");
  ...
  function looksCircular(element) {
    ...
    const rounded = /(?:50%|100%)/i.test(radius) ||
      /(?:9999|100)\s*px/i.test(radius);
    const sized = rect.width >= 16 && rect.width <= 112 &&
      rect.height >= 16 && rect.height <= 112 &&
  ...
  // Flow has changed the account trigger markup a few times. Only infer an
  // unlabelled avatar in an actual header, and require a circular visual plus
  // either an avatar hint or a short initial. This avoids locking round Help,
  // menu, or editor controls just because they happen to be circular.
  function isGenericAvatarControl(control) {
    ...
    const visualCandidates = [control, ...control.querySelectorAll(
      "img, [role='img'], svg, span, div, " + genericAvatarHintSelector
    )];
    const hasHint = control.matches(genericAvatarHintSelector) ||
      !!control.querySelector(genericAvatarHintSelector);
    const hasCircle = visualCandidates.some(looksCircular);
    if (!hasCircle) return false;
    if (hasHint) return true;
    // An unlabelled circular image in the header is also a useful account
    // signal; logos and ordinary header options are normally rectangular.
  ...
  function addAccountControl(controls, element) {
    ...
    else if (element.matches("[data-ogsr-up], .gb_A, .gb_B, .gb_d, .gb_Aa, .gb_Ca")) {
      // Google sometimes puts the identity trigger on a non-semantic wrapper.
  ...
  function collectLockedControls() {
    ...
    for (const element of document.querySelectorAll(accountSelector)) {
      addAccountControl(controls, element);
    }
    for (const avatar of document.querySelectorAll(avatarSelector)) {
      const trigger = avatar.closest(interactiveSelector);
      // data-gbid is the stable marker on Flow's own account avatar. Other
      // image heuristics are deliberately restricted to header controls.
      if (trigger && (
        avatar.matches("[data-gbid]") || isHeaderElement(trigger) ||
        avatar.matches("img[alt='User profile image' i]")
      )) controls.add(trigger);
    }
    const headerControls = document.querySelectorAll(
      "flow-app-header " + interactiveSelector + ", header " + interactiveSelector +
      ", [role='banner'] " + interactiveSelector
    );
    for (const control of headerControls) {
      if (isGenericAvatarControl(control)) controls.add(control);
    }
    return controls;
  }
  ...
  function historyContext(element, page) {
    ...
    while (parent && parent !== page) {
      ...
      if (/(?:project|history).*(?:list|grid|history|card)|(?:history|project).*(?:list|grid|card)/i.test(identity)) {
        return true;
      }
  ...
  function pageHasNewProject(page) {
    return newProjectButton(page) ||
      [...page.querySelectorAll(interactiveSelector)].some(button =>
        /new project/i.test(button.getAttribute("aria-label") || "") ||
        /new project/i.test(button.getAttribute("title") || "") ||
  ...
  function hideProjectHistory() {
    ...
    for (const page of document.querySelectorAll("flow-projects-page")) {
      ...
      for (const child of page.children) {
        // The dashboard's history/list region is a plain DIV in some Flow
        // builds. Require a project/history signal before hiding a generic
        // sibling, so unrelated dashboard options remain available.
        const identity = `${child.id || ""} ${child.className || ""}`;
        const historyLike = isProjectCard(child, page) ||
          child.matches(skeletonSelector) ||
          !!child.querySelector(projectCardSelector) ||
          (!!child.querySelector(skeletonSelector) &&
            (historyContext(child, page) || /(?:project|history)/i.test(identity))) ||
          /(?:project|history).*(?:list|grid|history|card)|(?:history|project).*(?:list|grid|card)/i.test(identity);
        if (hasNewProject && child.tagName === "DIV" && historyLike && safeToHide(child)) {
          hidden.add(child);
        }
      }
      ...
      for (const element of page.querySelectorAll(projectCardSelector)) {
        if (safeToHide(element)) hidden.add(element);
      }
      for (const skeleton of page.querySelectorAll(skeletonSelector)) {
        if (historyContext(skeleton, page) && safeToHide(skeleton)) hidden.add(skeleton);
      }
    }
    // Angular can recycle a list container when navigating into an editor.
    // Keep a reference as well as querying the document so a detached node
    // cannot carry our marker into a later reinsertion.
    for (const old of hiddenElements) {
      if (!hidden.has(old)) old.removeAttribute(marker);
    }
  ...
  function apply() {
    ...
    if (!style?.isConnected) {
      ...
      style.textContent = `html[${rootMarker}] [${marker}] {
          display: none !important; visibility: hidden !important; pointer-events: none !important;
        }
        html[${rootMarker}] :is(${accountSelector}),
        html[${rootMarker}] :is(${accountSelector}) *,
        html[${rootMarker}] [data-bf-flow-account-locked],
        html[${rootMarker}] [data-bf-flow-account-locked] * {
    ...
    for (const [control, original] of lockedControls) {
      if (!controls.has(control) || !control.isConnected) unlock(control, original);
    }
    for (const control of controls) {
      if (!lockedControls.has(control)) lockedControls.set(control, {
      ...
      // inert is enforced by Chrome, including descendants and embedded
      // account-widget frames. It keeps the avatar visible while inert.
      control.setAttribute("inert", "");
      control.setAttribute("data-bf-flow-account-locked", "");
    }
    hideProjectHistory();
  }

  function schedule() {
    if (destroyed || !managed || queued) return;
    queued = true;
  ...
  function onRuntimeMessage(message) {
    if (message?.type === "CHECK_CONTEXT") void check();
  }
  ...
  function onInteraction(event) {
    ...
    const blockedPath = path.some(node => {
      ...
      return node.matches(accountSelector) ||
        node.hasAttribute("data-bf-flow-account-locked") ||
        (isInteractive(node) && node.querySelector(avatarSelector));
    });
    // inert/pointer-events may retarget a real click to an ancestor behind
    // the avatar. Reject that physical click before delegated app handlers.
    const touches = event.touches?.[0] || event.changedTouches?.[0];
    const point = touches || event;
    const pointerEvent = !event.type.startsWith("key") &&
      event.type !== "beforeinput" &&
      (event.detail > 0 || !!touches || event.type.startsWith("pointer") ||
        event.type.startsWith("mouse") || event.type === "contextmenu");
    const blockedPosition = pointerEvent &&
    ...
    if (blockedPath || blockedPosition) {
  ...
  observer.observe(document, {
    ...
    attributeFilter: [
      "aria-label", "title", "href", "src", "class", "id",
      "style", "data-ogsr-up", "data-gbid", "data-testid"
    ]
  });
