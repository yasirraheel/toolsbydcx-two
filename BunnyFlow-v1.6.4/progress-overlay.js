// Progress UI with an explicit resume request. The worker authorizes the
// active tab before performing any protected action.
(() => {
  if (globalThis.__bunnyFlowProgressOverlay) return;
  globalThis.__bunnyFlowProgressOverlay = true;
  let timer = null;
  let host = null;
  let dismissed = false;
  let pending = false;
  ...
  function progressKey(progress) {
    return [progress.state, progress.percent, progress.label || ""].join("\u001f");
  }
  function rememberProgress(progress) {
    ...
    if (changed) {
      ...
      resumeError = "";
      // A changed step is evidence that a previous resume request was acted
      // on. Let the next state decide whether the button should be shown.
      resumePending = false;
    }
    return changed;
  }
  function stalled(progress) {
    return progress.state === "running" && lastProgressKey !== null &&
      Date.now() - lastProgressAt >= STALL_AFTER;
  }
  function recoverFirstIdentifier(progress) {
    // The overlay starts even when automation's browser handshake was missed.
    // Only the visibly rendered first email form may request a recovery.
    if (progress.percent !== 10 ||
        !["error", "attention"].includes(progress.state) && !stalled(progress) ||
  ...
  // Worker progress is more detailed for diagnostics than this customer UI.
  function displayLabel(progress) {
    if (progress?.state === "attention" || progress?.state === "manual") {
      ...
      if (detail.includes("verification") || detail.includes("code")) {
        return "Enter the verification code shown or required by Google";
      }
  ...
  function resumeFailure(error) {
    const message = error instanceof Error ? error.message : responseMessage(error);
     return message || "Setup needs attention. Try Resume setup.";
  }
  async function resumeLogin() {
    ...
    ui.resume.textContent = "Resuming…";
     ui.label.textContent = "Setting things up…";
    try {
      ...
      // The request is finished; don't lock the button forever if the retry
      // reaches the same error/step before the next progress poll.
      resumePending = false;
      lastProgressAt = Date.now();
    } catch (error) {
      if (!active || requestEpoch !== epoch || dismissed || requestProgressKey !== lastProgressKey) return;
      ...
      ui.label.textContent = resumeError;
      ui.card.dataset.state = "error";
      ui.resume.hidden = false;
      ui.resume.disabled = false;
     ui.resume.textContent = "Resume setup";
    }
  }
  function build() {
    host = document.createElement("div");
    host.id = "bunnyflow-login-progress";
    ...
    shadow.innerHTML = `<style>
      ...
      .card{background:#1e1c24;color:#edeaf2;border:1px solid #544562;border-radius:10px;padding:12px;box-shadow:0 4px 18px #0003;max-height:240px;overflow:auto}
      header{display:flex;align-items:center;gap:8px;margin-bottom:8px}strong{font-size:12px} .percent{margin-left:auto;font-weight:600;font-size:13px;color:#c9b3e8}
      button{background:transparent;border:0;color:#cec3da;font-size:20px;cursor:pointer;padding:0 0 0 8px}
      p{margin:0 0 9px;overflow-wrap:anywhere} .track{height:8px;background:#362842;border-radius:6px;overflow:hidden}
      .fill{height:100%;width:0;background:#ac62ef;transition:width .25s ease}
      small{display:block;margin-top:8px;color:#b9aec5;font-size:11px}
      .resume{display:block;width:100%;margin-top:10px;background:#ac62ef;border:1px solid #c88aff;border-radius:6px;color:#fff;font:600 12px/1.3 system-ui,sans-serif;cursor:pointer;padding:7px 10px}
      .resume:hover{background:#b977f0}.resume:disabled{cursor:wait;opacity:.75}
      .resume[hidden]{display:none}
      [data-state="complete"] .fill{background:#4acc99}[data-state="attention"] .fill,[data-state="error"] .fill{background:#efb359}
      @media(prefers-reduced-motion:reduce){.fill{transition:none}}
    </style><div class="card"><header><strong>BunnyFlow</strong><span class="percent"></span><button class="dismiss" type="button" aria-label="Hide workspace setup progress">×</button></header>
  ...
  async function refresh() {
    ...
    try {
      ...
      if (progress.state === "complete") {
        resumeError = "";
        resumePending = false;
        ui.label.textContent = "Your workspace is ready";
        ui.card.dataset.state = "complete";
      }
      setResumeButton(progress);
    } catch {
      if (ui && active && requestEpoch === epoch) {
        ui.label.textContent = "Setup needs attention";
        ui.card.dataset.state = "error";
        resumeError = "Setup needs attention. Try Resume setup.";
  ...
  window.addEventListener("pagehide", () => {
    active = false; epoch++;
    pending = false;
    clearInterval(timer); timer = null; remove();
  });
  window.addEventListener("pageshow", () => { active = true; start(); });
  Promise.resolve(globalThis.flowAutoLoginBrowserReady ?? globalThis.flowAutoLoginIsEdge)
    .then(value => {
      supported = value === true;
      if (supported) start();
      else globalThis.__bunnyFlowProgressOverlay = false;
    }).catch(() => { globalThis.__bunnyFlowProgressOverlay = false; });
})();