...
function dayHours($targetDateTime = '2023-11-01 18:00:00', $currentDateTime = null)
{
    $targetDateTime = Carbon::parse($targetDateTime);
    $currentDateTime = $currentDateTime ? Carbon::parse($currentDateTime) : Carbon::now();
    $totalHoursDifference = $currentDateTime->diffInHours($targetDateTime);
    $daysDifference = intdiv($totalHoursDifference, 24);
    $hoursDifference = $totalHoursDifference % 24;
    return "{$daysDifference}D {$hoursDifference}H";
}

function listingProgressbar($accountListing, $stage, $getBoolean = false)
{
    if ($getBoolean) {
        return @$accountListing->step >= $stage;
    }
    return @$accountListing->step >= $stage ? 'active' : '';
}

function userNotifyPermission($user, $act)
{
