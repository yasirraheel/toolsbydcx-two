...
const DCX_DASHBOARD_URL = "https://toolsbydcx.com/dashboard";
...
export function allowedNavigation(value) {
  let dcxSite = false;
  try { dcxSite = ["https://toolsbydcx.com", "https://www.toolsbydcx.com"].includes(new URL(value).origin); }
  catch {}
  return isFlowUrl(value) || dcxSite || isGoogleLoginUrl(value) || isChromeWebStoreUrl(value);
}
...
export function createNavigationRules() {
  return [
    {
      id: 1001, priority: 10,
      action: { type: "redirect", redirect: { url: DCX_DASHBOARD_URL } },
      condition: { resourceTypes: ["main_frame"], regexFilter: "^https://(?:myaccounts?\\.google\\.com|drive\\.google\\.com|mail\\.google\\.com|(?:www\\.)?gmail\\.com)(?:[/:?#]|$)" }
    },
    {
      id: 1002, priority: 10,
      action: { type: "redirect", redirect: { url: DCX_DASHBOARD_URL } },
